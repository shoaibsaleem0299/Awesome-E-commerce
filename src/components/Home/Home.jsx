...
const Home = () => {
  ...
  const [initialProductShow, setinitialProductShow] = useState(8);
  const productsOnHome = products.slice(0, initialProductShow);
  const handleVeiwMore = () => {
    if (initialProductShow < products.length) {
      setinitialProductShow(initialProductShow + 8);
    }
  }
    ...
    <>
      <div className='bg-gray-100 py-4'>
        ...
        <div className='w-[95%] mx-auto grid justify-items-center grid-cols-1 md:grid-cols-2 lg:grid-cols-4 '>
          {productsOnHome.map((product, index) => (
